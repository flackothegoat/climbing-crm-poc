...
> 当前阶段：云部署与区域 A/B 测试已告一段落；尚未去岩馆现场确认，也尚未购买摄像头、边缘计算设备或岩点拍摄设备。

## 1. 一句话说明当前状态

当前 POC 已经把“岩点资产—W06 虚拟定线—整馆墙面查看—线路运营统计”做成可演示流程；2026-08-06 已在本地完成墙面、孔位、线路版本、安装位置和攀爬事件的最小持久化骨架，W06 定线与人工事件刷新后不再丢失。该切片尚未部署到 Azure，云端 Southeast Asia 仍运行上一版本；墙面和线路数据仍属于测绘估算或 Dummy，不代表现场确认。
...
- 红、绿、黄三个经过清理的独立 GLB 岩点。
- 岩点拖拽、最近孔位吸附、绕墙面法线旋转。
- 演示性主安装孔与墙孔匹配。
- 基于岩点投影半径的碰撞提示。
- 红、绿、黄三条测试线路，以及起点、普通点和终点。
- 整线高亮、整线拆卸、单点删除、撤销和恢复演示数据。
...
- 三个清理模型仍通过固定 `demoAssetKey` 和静态 URL 引用，并明确标记为 `DUMMY`；尚未与数据库中的正式 `HoldVariant.id` / `HoldAsset.id` 建立关系。
- 保存 Dummy 定线草稿不会改变岩点库存；正式安装/拆卸命令及其库存同事务联动尚未实现。
...
| 可交互业务原型 | W06 定线、整馆查看、墙段选择、线路统计 | 能证明流程，但尚未进入正式数据库 |
...
当前 Prisma 共 14 个迁移，最后两个目录是：

- `/Users/flacko/Documents/Codex/SummerIntern/poc/apps/api/prisma/migrations/20260806103000_wall_route_observation_foundation`
- `/Users/flacko/Documents/Codex/SummerIntern/poc/apps/api/prisma/migrations/20260806113000_expand_route_placement_rotation`
...
2. 补充线路发布/退役的显式状态迁移 API，确保发布版本不可原地覆盖。
3. 设计正式“确认安装/拆卸”命令，把 `RouteHoldPlacement`、`WAREHOUSE ↔ INSTALLED`、库存流水和审计放入同一事务。
4. 将整馆墙段覆盖层和平面折线也改为以 `/api/walls` 为数据源，减少前后端重复常量。
...
当前尚未现场确认，也没有购买摄像头或边缘硬件。不要开始摄像头识别。
先以当前代码为事实来源，检查本地状态。本地已完成 Wall / Route / RouteHoldPlacement / ClimbObservation 的第一版持久化，W06 保持明确的 SURVEY_ESTIMATE / DUMMY 标记；云端尚未部署。下一步优先检查 L1/L2 浏览器行为、补线路状态迁移，并设计正式安装/拆卸与库存同事务命令。不要改动 Azure 部署和 27 MB 模型加载方式，除非我另行要求。
```

...
>
> 稳定基线：本文档所在提交；应用发布基线另由发布标签锁定，包含本地 Vision Worker、识别历史、可恢复录像证据、人工复核、单线路双来源复盘、线路管理界面优化和无人画面误报防护
>
> 部署边界：Azure 已部署 `release-2026-09-16` / `a639cdf`；完整验证、备份与回退点见 `docs/发布记录-2026-09-16.md`
...
| 项目     | 本地                                                | Azure 生产                                                  |
| -------- | --------------------------------------------------- | ----------------------------------------------------------- |
| 代码     | 本地 `main`；发布后可只增加文档提交                 | `release-2026-09-16` / `a639cdf`，由 `DEPLOYED_COMMIT` 确认 |
...
- Prisma 识别到 35 个 migration，生产数据库 schema 已是最新状态；31 类数据库不变量异常计数均为 0。
...
当前 Web/API/Worker 均健康，Worker 成功读取 2 条已发布线路。登录入口已使用当前核心能力重新表达，并完成桌面与窄屏视觉验收。本次没有改动录像状态机；2026-09-15 发布时确认的历史 `NOT_RECORDED` 记录仍不会由新版本补造录像。详情以 `docs/发布记录-2026-09-16.md` 和前一日发布记录为准。
...
本次发布候选基线验证（2026-09-15）：

- `pnpm lint`：通过。
- `pnpm typecheck`：通过。
- `pnpm test`：通过；当前为 API 128 项、Web 37 项，共 165 项单元测试通过；8 项显式数据库集成测试按默认策略跳过。
- `pnpm worker:test`：29 项 Worker 测试通过。
- `pnpm build`：API 和 Web 生产构建通过。
- 本地 migration 已应用至 `20260915180000_camera_observation_evidence_lifecycle`，共 35 个；31 类数据库不变量全部为 0。

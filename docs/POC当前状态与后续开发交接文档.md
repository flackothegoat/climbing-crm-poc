...
>
> 部署边界：本地、Azure 和 GitHub 是三个独立环境；实际生产版本、验证结果和回退点应查阅对应的发布记录
...
| 项目     | 本地                                                | Azure 生产                                             |
| -------- | --------------------------------------------------- | ------------------------------------------------------ |
| 代码     | 本地 `main` 与功能分支按发布流管理                  | 以 `/opt/climbing-demo/DEPLOYED_COMMIT` 和发布记录为准 |
...
- `pnpm worker:test`：27 项 Worker 测试通过。

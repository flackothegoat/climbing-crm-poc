...
>
> 部署边界：Azure 已部署 `release-2026-09-15` / `54cea43`；完整验证、备份与回退点见 `docs/发布记录-2026-09-15.md`
...
| 项目     | 本地                                                | Azure 生产                                                  |
| -------- | --------------------------------------------------- | ----------------------------------------------------------- |
| 代码     | 本地 `main`；发布后可只增加文档提交                 | `release-2026-09-15` / `54cea43`，由 `DEPLOYED_COMMIT` 确认 |
...
正式发布后 Web/API/Worker 均健康，Worker 成功读取 2 条已发布线路并解码实时流。生产库已有的 78 条识别记录均为历史 `NOT_RECORDED`，不会由新版本补造录像；新录像状态机从下一条真实识别记录开始生效。详情以 `docs/发布记录-2026-09-15.md` 为准。

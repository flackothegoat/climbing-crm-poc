...
> 当前阶段：第一阶段“线路数字化 + 二维码反馈”纵向闭环已在本地实现；尚未去岩馆现场确认。TL-IPC48AN 已在公司网络跑通“摄像头 → Wi-Fi → 公网 → Azure GB28181 平台 → 浏览器实时显示”，岩馆 Wi-Fi、出网和公网 IP 状态仍待现场验证。

## 1. 一句话说明当前状态

当前产品主线已经调整为“线路建档—发布二维码—会员匿名反馈—线路表现复盘”。本地已完成不依赖 GLB/孔位的跨墙段线路档案、版本发布与下线、线路照片、签名公开链接、10 秒反馈、样本口径、透明建议和历史保留；原 W06 虚拟定线与岩点库存联动保留为独立实验能力。该切片尚未部署到 Azure；墙面与孔位仍属于测绘估算，二维码反馈也只代表主动反馈样本，不得称为全馆真实尝试或真实完攀率。
...
项目第一阶段先建立线路运营数据入口，第二阶段再增强岩点与三维定线：
...
墙面展示入口：`/dashboard/assets/walls`。第一阶段线路运营入口：`/dashboard/assets/routes`；数据复盘入口：`/dashboard/data`；会员公开反馈入口：`/r/:token`。

第一阶段线路运营已形成独立纵向闭环：

- 不依赖 GLB、孔位或岩点安装坐标即可创建跨一个或多个墙段的线路档案。
- 草稿可记录难度、颜色、风格、定线员、计划拆线日、说明和线路照片。
- 发布生成可打印、可重复生成的签名二维码；会员无需登录即可匿名反馈。
- 数据页按线路版本统计样本量、反馈者完攀比例、难度偏差、喜好和安全疑虑，并明确样本口径。
- 线路下线后公开入口停止收集新反馈，但发布版本、照片和历史反馈保留。
- W06 三维定线迁到 `/dashboard/assets/routes/setting`，作为第二阶段实验能力，不再阻塞线路运营主流程。
...
| 层级             | 当前内容                                                                         | 使用边界                                                   |
| ---------------- | -------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| 正式业务实现     | 认证、会话、员工、岩点资产、库存、线路建档/发布/下线、二维码反馈、版本复盘和审计 | 可继续作为后续模块的全栈基础；二维码指标仅代表主动反馈样本 |
| 可持久化业务骨架 | W06 定线任务、岩点预留与安装完成                                                 | 已进入本地正式表，墙面几何仍待现场                         |
...
当前 Prisma 共 25 个迁移，最后三个目录是：

- `/Users/flacko/Documents/Codex/SummerIntern/poc/apps/api/prisma/migrations/20260818170000_route_feedback_vertical_slice`
- `/Users/flacko/Documents/Codex/SummerIntern/poc/apps/api/prisma/migrations/20260820162000_route_visual_annotations`
- `/Users/flacko/Documents/Codex/SummerIntern/poc/apps/api/prisma/migrations/20260821115000_unified_climbing_color`
...
2026-08-18 第一阶段线路闭环验证：ESLint、全仓 TypeScript、API 81 项单元测试、Web 26 项测试、5 项 PostgreSQL 租户/领域约束集成测试、生产构建和格式检查通过；23 个 migration 全部应用且 schema 同步，14 类数据库不变量均为 0。浏览器使用本地测试组织实测了“创建轻量墙段 → 线路建档与照片上传 → 发布签名二维码 → 无登录匿名反馈 → 数据看板更新”，样本量和指标口径正确；下线撤销公开链接、保留历史反馈由事务单元测试覆盖。只修改本地 feature 分支与本地 PostgreSQL，未修改 `main` 或 Azure。
...
- 已取得 TP-LINK TL-IPC48AN 试验摄像头，并在公司网络跑通 GB28181 主动注册到 Azure 与浏览器实时显示；岩馆网络、机位和视频质量尚未验证，也尚未购买边缘计算机、标定板或岩点拍摄台。
...
### 11.4 已完成：第一阶段线路数字化与二维码反馈纵向闭环

1. 普通线路可只关联一个或多个墙段建档，不再要求 GLB、几何版本、孔位或岩点位置。
2. 线路草稿记录编号、名称、颜色、难度、评级体系、风格、定线员、计划拆线日期、说明和照片。
3. L1 发布线路时生成签名公开 token；数据库只保存 token 哈希和 link id，可重复生成相同二维码。
4. 会员无需登录即可提交完攀状态、难度偏差、喜好、安全疑虑和可选短建议。
5. 匿名会话标识只保存哈希，同设备六小时内再次提交更新本轮反馈；公开入口带 IP/token 限流和幂等键。
6. 线路下线后二维码停止接收反馈，但版本和历史反馈保留。
7. 运营看板明确显示样本量、反馈者完攀比例、难度偏差、喜欢比例、安全疑虑和透明规则建议。
8. `RouteFeedback` 与 `ClimbObservation` 分离：前者是主动反馈，后者继续承接 MANUAL/CAMERA 观察事件。
9. 原 3D 定线工作台迁到独立实验入口，避免阻塞第一阶段线路运营。

### 11.5 摄像头链路最新状态

- 试验型号：TP-LINK `TL-IPC48AN`。
- 公司网络已验证 GB28181 主动注册到 Azure 平台，并能从浏览器显示实时视频。
- 已验证链路：摄像头 → Wi-Fi → 公网 → Azure → 浏览器。
- 尚未验证：岩馆 Wi-Fi 覆盖与稳定性、上行带宽、NAT/公网 IP 状态、实际机位遮挡、逆光、夜间画质、时间同步和隐私告知。
- 此状态只证明传输链路可用，不代表尝试/完攀算法已具备可靠精度。

### 11.6 已完成：W04/W05 线路视觉定位纵向闭环

1. 新增 `RouteVisualAnnotation` / `RouteVisualPoint`，以墙段局部 `0..1` 坐标保存线路视觉点，不绑定整体 GLB 的单网格拓扑。
2. 视觉标注采用独立修订状态机：草稿不会污染正式展示；确认新修订时，原确认修订自动退役。
3. API 和数据库同时校验：视觉点只能引用该线路版本已关联的墙段，且不能跨组织写入。
4. 普通线路建档新增同墙区、相邻墙段校验；线路位置按有序 `W04 → W05` 表达，而不是把线路强行归属到单墙。
5. 线路运营页新增 3D 扫描模型高亮、可编辑正立面线路图和现场画面状态页；W04/W05 共用同一条跨墙线路。
6. 会员二维码页展示已确认的线路正立面；未确认草稿不会被公开读取。
7. 现场画面页只展示真实接入状态，当前明确为“等待岩馆验证 / 算法未启用”，不使用演示视频冒充实时分析。
8. 本地人工验收建立并发布了 `W04-BLUE` 验收线路，完成 W04 起点、W05 终点、保存、确认、发布和公开页展示全流程；该数据只属于本地测试组织。
9. 迁移前备份：`backups/poc/postgres-pre-route-visual-20260820.dump`；迁移 `20260820162000_route_visual_annotations` 已应用到本地 PostgreSQL，Azure 未改动。

### 11.7 已完成：统一线路与岩点业务颜色

1. 新增 `ClimbingColor`：黑、白、灰、红、橙、黄、绿、蓝、紫、粉、棕共 11 个常见颜色，API 和 PostgreSQL 只存英文枚举键。
2. 线路、线路版本、岩点规格和 3D 定线资产统一引用同一枚举；颜色不是线路身份，线路仍以 `routeId` 和版本标识。
3. 前端取消 RGB/Hex 调色板，统一显示中文颜色名和固定色块；CSS 色值只存在展示层。
4. 历史中文名、英文名和 Hex 已由迁移归并；未知旧值保守映射为灰色。
5. 活动岩点档案使用 `(holdModelId, activeColor)` 唯一约束；删除后将 `activeColor` 释放为 `NULL`，保留审计存根。
6. 迁移前备份：`backups/poc/postgres-pre-unified-color-20260821.dump`；迁移已应用到本地 PostgreSQL，Azure 未改动。

### 11.8 建议下一项任务

1. 本地测试组织已人工走通创建墙段、建档、上传照片、发布、公开页提交和看板更新；下一步由产品侧按真实线路口径做一轮验收并确认表单字段。
2. 到岩馆选择一面墙和 5–10 条线路做二维码试点，确认线路编号、评级体系、风格标签、定线员和换线节奏。
3. 先积累真实二维码反馈，校准问卷完成时间、张贴位置、反馈率和样本阈值。
4. 现场验证 TL-IPC48AN 的网络、机位和视频质量，采集获授权样例，再决定摄像头事件算法优先级。
5. 经用户确认后再规划 Azure 部署；部署前必须备份正式数据库并先运行 migration 检查。
6. 第二阶段再恢复正式拆卸/换点、多孔安装型式、轻量 3D 和跨相邻墙段精确几何工作。

### 11.8.1 已完成：W03–W05 试点扫描模型的线路颜色隔离

1. 试点资产改用现场重扫的 `w3-w4-w5.glb`，代码内副本位于 `apps/web/public/walls/regions/w03-w05-pilot.glb`，SHA-256 为 `0f4f9cf9bd2a16cd86c71e188da6ee57c2f9ff81fb788e3c07e7fd21cb1a36e7`。
2. W03、W04、W05 的模型局部边界与颜色分析参数集中在 `apps/web/src/features/routes/route-visual-region.ts`；这些是试点校准数据，不散落在渲染组件，也不冒充现场精确测绘。
3. 线路视觉工作台按当前线路的统一业务颜色和关联墙段共同筛选：匹配岩点保留扫描原始纹理、叠加黄色轮廓；其他区域灰度显示。没有把岩点重新染成线路 CSS 色值。
4. 线路为蓝色时只分析该线路关联的 W04/W05，不会把 W03 的同色岩点误归入跨墙线路。红、橙、黄、绿、蓝、紫、粉、棕使用同一套通用 Shader；黑、白、灰因纹理低饱和度易与墙体混淆，当前明确回退为灰度模型加人工确认点。
5. 正式展示不再画虚构的彩色折线或给所有标注点染色；只显示来自已确认视觉修订的 S（起点）和 T（终点）。已有本地 `W04-BLUE` 的两个 50%/50% 点只是旧流程验收坐标，去岩馆前仍需按真实岩点重新标记。
6. 当前自动效果是浏览器端基于扫描纹理的颜色候选分割，不是已经训练完成的 AI 实例识别：它能形成近似黄色轮廓，但不会把结果持久化为独立岩点实例，也不能可靠处理阴影、反光、低饱和色和相邻同色体积。
7. 后续上传边界：原始 GLB 属于跨墙段 `WallCapture`，必须通过 `WallCaptureSegment` 关联多个墙段；分析产物应另存为派生资产及候选实例，人工确认后才生成正式线路视觉标注。不要把区域 GLB 错存为某一个岩点资产，也不要直接覆盖已确认修订。
8. 本次只修改本地 feature 分支和静态试点资产；没有迁移数据库，没有改动本地 PostgreSQL，也没有部署 Azure。
...
当前尚未去岩馆现场确认。TL-IPC48AN 已在公司网络通过 GB28181 主动注册 Azure 并能在浏览器显示实时视频，但岩馆 Wi-Fi、出网、机位和公网 IP 状态未验证；不要在没有现场样例前开发完攀识别。
先以当前代码为事实来源，检查本地状态。本地已完成第一阶段线路建档、发布二维码、匿名反馈、线路复盘和下线历史闭环；二维码反馈与 ClimbObservation 分表。W04/W05 已完成独立修订的线路视觉标注、正立面与会员公开展示；W03–W05 试点已切换为 5 MB 现场重扫模型，并完成按线路颜色与墙段过滤的原色岩点、黄色轮廓和背景灰度 Shader。本地 W04-BLUE 只用于验收，其 50%/50% S/T 坐标仍需现场重标。W06 3D 定线、岩点库存预留和安装继续作为第二阶段实验能力；云端尚未部署。下一步先完成区域 GLB 上传、派生分析资产和人工确认边界，再去岩馆校准视觉点与摄像头观察区。不要改动 Azure 部署，除非我另行要求。
```
...
2026-08-18 已在 `feature/route-core-replan` 本地实现第一阶段线路纵向闭环，新增 1 个 migration、线路运营与公开反馈 API、线路照片、二维码、10 秒移动反馈页和样本口径看板；迁移前备份位于 `backups/poc/postgres-pre-route-feedback-20260818.dump`。摄像头公司网络 GB28181 链路状态已更新。没有修改 Azure 资源；后续从第 11.6 节继续。

2026-08-20 已在同一 feature 分支完成 W04/W05 线路视觉定位：新增版本化标注模型、3D/正立面/现场状态工作台、跨相邻墙段校验和会员公开线路图。本地已完成 `W04-BLUE` 创建、标注、确认、发布和二维码展示验收；迁移前备份位于 `backups/poc/postgres-pre-route-visual-20260820.dump`。只修改本地代码和本地 PostgreSQL，Azure 未变。

2026-08-21 已统一线路、线路版本、岩点和 3D 定线资产的颜色模型：后端/数据库使用 `ClimbingColor`，前端只提供 11 个中文业务颜色选项，不再提交 RGB/Hex。迁移前备份位于 `backups/poc/postgres-pre-unified-color-20260821.dump`，25 个迁移均已应用到本地 PostgreSQL；Azure 未变。

2026-08-24 已接入 W03–W05 现场重扫试点模型，完成按线路业务颜色与关联墙段联合过滤的“原始纹理 + 黄色轮廓 + 其他区域灰度”通用 Shader，并将展示标记统一为 S/T。前端类型检查、31 个测试、生产构建及本地浏览器 W04/W05 跨墙验收通过；未修改数据库和 Azure。

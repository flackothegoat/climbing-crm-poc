...
> 当前分支：`main`
>
> 稳定基线：本文档所在提交，包含本地 Vision Worker、识别历史、录像证据、人工复核、单线路双来源复盘和线路管理界面优化
>
...
- 线路库采用“档案列表 + 右侧小概况”的紧凑布局；编辑信息在独立弹窗内完成，不再把长表单插到页面顶部。
- 线路复盘首屏展示线路图片和难度、颜色、墙段、状态，帮助用户把数据与墙面线路对应起来；没有图片时明确显示空状态。
...
- 本地浏览器已验证线路库紧凑布局、编辑信息弹窗、线路图片/空图展示和简化后的复盘文案。

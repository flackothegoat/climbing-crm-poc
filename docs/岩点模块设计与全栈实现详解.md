...
当前代码用产品归一键和统一业务颜色枚举表达业务身份：

```text
productKey = 生产商 | 产品名称 | 尺寸等级 | 固定方式 | 宽 | 高 | 深
color      = BLACK | WHITE | GRAY | RED | ORANGE | YELLOW | GREEN | BLUE | PURPLE | PINK | BROWN
```

`productKey` 每一段都会先去除首尾空格、合并连续空格并转为小写。颜色不是 RGB，也不允许任意文本；API 和 PostgreSQL 共用 `ClimbingColor` 枚举，中文名称和固定色块只由前端展示层映射。数据库唯一约束保证：

- 同一用途分类下，`productKey` 不能重复。
- 同一产品型号下，活动档案的 `color` 不能重复。
...
- `(holdModelId, activeColor)`：同型号下不重复创建相同活动颜色档案。
...
活动唯一键使用可空的 `activeColor`。删除时将它设为 `NULL`，PostgreSQL 允许多个 `NULL`，因此旧档案仍保留，而相同业务颜色的新档案可以重新建立。系统不再区分“同名不同 RGB 色阶”；如现场以后确有新的运营颜色，应先扩充统一枚举和展示映射，再新增数据。
